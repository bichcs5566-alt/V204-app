...
def ensure_dashboard_dir():
    ...
    watchlist_template = DASHBOARD_DATA_DIR / "watchlist.csv"
    if not watchlist_template.exists():
        pd.DataFrame(columns=["stock_id"]).to_csv(watchlist_template, index=False)
...
def select_stocks(d):
    ...
    if len(core) < MIN_CORE_FILL:
        need = MIN_CORE_FILL - len(core)
        extra = core_fallback[~core_fallback["stock_id"].isin(set(core["stock_id"]))].head(need)
    ...
    if len(alpha) < MIN_ALPHA_FILL:
        need = MIN_ALPHA_FILL - len(alpha)
        extra = alpha_fallback[~alpha_fallback["stock_id"].isin(set(alpha["stock_id"]))].head(need)
