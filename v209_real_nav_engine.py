...
def load_positions():
    ...
    if "trade_date" not in pos.columns:
        raise ValueError("v202_positions.csv 必須包含 trade_date 欄位")
    ...
    daily = (
        pos.groupby("trade_date", as_index=False)
          .apply(lambda g: pd.Series({
              ...
              "avg_exposure": float(g["weight"].fillna(0).sum())
          }))
...
def rebuild_nav_from_returns(base_nav, daily_ret_df):
    ...
    daily_ret_df = daily_ret_df.rename(columns={"trade_date": "date"})
...
def build_tiers(pos):
    ...
    if "signal_date" in df_to_save.columns:
        df_to_save["signal_date"] = pd.to_datetime(df_to_save["signal_date"], errors="coerce").dt.strftime("%Y-%m-%d")
    df_to_save.to_csv(TIER_MAP["all"], index=False)
    ...
    for tier in ["lt10", "10_30", "30_50", "50_100", "100p", "unknown"]:
        ...
        sub_save = sub.copy()
        sub_save["trade_date"] = sub_save["trade_date"].dt.strftime("%Y-%m-%d")
        ...
        overview_rows.append({
            ...
            "latest_trade_date": str(sub["trade_date"].max().date()) if "trade_date" in sub.columns and len(sub) else "",
            "latest_signal_date": (
...
def main():
    ...
    print("v209 DONE")
    print("已用 positions 權重 × day_ret 重建真實 NAV")
