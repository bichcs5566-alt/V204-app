# v249_dual_regime_engine.py
# v249 雙策略版（Hybrid / Dual Regime）
# 目標：
# 1) 嚴格 T+1，不偷看
# 2) 市場趨勢盤 → aggressive
# 3) 震盪 / 弱勢盤 → defensive
# 4) 保留風控骨架，避免 v241 式爆倉
# 5) 提高報酬效率與 Sharpe
...
# Aggressive regime
AGG_TOP_N = 8
AGG_MAX_EXPOSURE = 0.90
AGG_MIN_EXPOSURE = 0.40
...
# Defensive regime
DEF_TOP_N = 4
DEF_MAX_EXPOSURE = 0.55
DEF_MIN_EXPOSURE = 0.20
DEF_MAX_POSITION_WEIGHT = 0.16
DEF_STOP1 = -0.04
DEF_STOP2 = -0.07
DEF_ADD1 = 0.06
DEF_ADD2 = 0.12
...
def load_price_panel():
    ...
    for req in ["symbol", "close"]:
        if req not in df.columns:
            raise ValueError(f"price_panel_daily.csv 缺少 {req} 欄位")
...
def build_market_regime(df):
    ...
    mkt["market_ma20"] = mkt["market_close"].rolling(20, min_periods=20).mean()
    mkt["market_ma60"] = mkt["market_close"].rolling(60, min_periods=60).mean()
    mkt["market_ma120"] = mkt["market_close"].rolling(120, min_periods=120).mean()

    # trend switch
    strong = (mkt["market_close"] > mkt["market_ma20"]) & (mkt["market_ma20"] > mkt["market_ma60"])
    weak = (mkt["market_close"] < mkt["market_ma120"])
    # 1=aggressive, 0=defensive, -1=deep weak defensive
    mkt["market_regime"] = np.where(strong, 1, np.where(weak, -1, 0))
    return mkt[["trade_date", "market_close", "market_ma20", "market_ma60", "market_ma120", "market_regime"]]
...
def regime_params(market_regime, macro_score):
    # Decide aggressive vs defensive
    if market_regime == 1:
        mode = "AGG"
        top_n = AGG_TOP_N
        ...
        addm1 = AGG_ADD_MULT1
        addm2 = AGG_ADD_MULT2
        base = 0.95 if macro_score > 0.5 else (0.80 if macro_score > 0 else 0.60)
        base_exposure = max(min(base, max_exposure), min_exposure)
    else:
        mode = "DEF"
        top_n = DEF_TOP_N
        ...
        addm1 = DEF_ADD_MULT1
        addm2 = DEF_ADD_MULT2
        base = 0.50 if macro_score > 0.5 else (0.40 if macro_score > 0 else 0.25)
        if market_regime == -1:
            base = min(base, 0.35)
        base_exposure = max(min(base, max_exposure), min_exposure if base > 0 else 0.0)
...
def rank_day(day_df, mode):
    ...
    if mode == "AGG":
        ...
        d["score"] = d["mom20"] * 0.35 + d["mom10"] * 0.40 + d["mom5"] * 0.25
    else:
        # 防守盤要求品質更高、波動較低
        d = d[(d["mom20"] > 0)].copy()
        d = d[d["vol20"].fillna(999) < d["vol20"].quantile(0.7) if len(d["vol20"].dropna()) > 10 else True]
        d["score"] = d["mom20"] * 0.55 + d["mom10"] * 0.30 + d["mom5"] * 0.15
...
def build_signals_for_next_day(day_df, holdings, nav_basis, dd_now, hard_mode):
    ...
    cfg = regime_params(market_regime, macro_score)
    ...
    for sym, pos in list(holdings.items()):
        ...
        if pnl <= cfg["stop1"] and (not pos["stop1_hit"]):
            pos["weight_mult"] *= 0.6 if cfg["mode"] == "AGG" else 0.5
            pos["stop1_hit"] = True
    ...
    allow_new_positions = (risk_mode != "NAV_HARD")
    if allow_new_positions and len(active_set) < cfg["top_n"]:
...
def build_summary(nav, trades, meta):
    ...
    # regime days summary
    if "strategy_mode" in nav.columns:
        agg_ratio = (nav["strategy_mode"] == "AGG").mean()
        def_ratio = (nav["strategy_mode"] == "DEF").mean()
    else:
        agg_ratio = np.nan
        def_ratio = np.nan
    ...
    return pd.DataFrame([{
        ...
        "agg_day_ratio": agg_ratio,
        "def_day_ratio": def_ratio,
