...
MIN_DAILY_TURNOVER = 3_000_000.0
...
NAV_MEDIUM_DD = -0.15
NAV_HARD_DD = -0.20
NAV_SOFT_EXPOSURE_CAP = 0.45
NAV_MEDIUM_EXPOSURE_CAP = 0.25
NAV_HARD_EXPOSURE_CAP = 0.00
REENTRY_DD = -0.05
...
def load_price_panel():
    ...
    if not path.exists():
        raise FileNotFoundError(f"ç¼ºå° {PRICE_FILE}")
    df = pd.read_csv(path)
    df.columns = [str(c).lower().strip() for c in df.columns]
    date_col = next((c for c in ["trade_date", "date", "datetime", "signal_date"] if c in df.columns), None)
    if date_col is None:
        raise ValueError("price_panel_daily.csv ç¼ºå°æ¥ææ¬ä½")
    if "symbol" not in df.columns or "close" not in df.columns:
        raise ValueError("price_panel_daily.csv éè¦ symbol / close æ¬ä½")
...
def load_macro():
    ...
    if not path.exists():
        raise FileNotFoundError(f"ç¼ºå° {MACRO_FILE}")
    ...
    if "trade_date" in macro.columns:
        ...
        raise ValueError("macro_signal.csv ç¼ºå° trade_date / date æ¬ä½")
    if "macro_score" not in macro.columns:
        raise ValueError("macro_signal.csv ç¼ºå° macro_score æ¬ä½")
...
def build_market_regime(df):
    ...
    cond_bear = mkt["market_close"] < mkt["market_ma60"]
    cond_weak = mkt["market_close"] < mkt["market_ma20"]
    mkt["market_regime"] = np.where(cond_bear, -1, np.where(cond_weak, 0, 1))
    return mkt[["trade_date", "market_close", "market_ma20", "market_ma60", "market_regime"]]
...
def merge_and_filter(price, macro, start, end):
    ...
    if filtered.empty:
        raise ValueError("ç¯©é¸å¾è³æçºç©º")
...
def get_base_exposure(macro_score, market_regime):
    if market_regime == -1:
        return 0.0
    regime_cap = 0.35 if market_regime == 0 else MAX_GROSS_EXPOSURE
    if macro_score > 0.5:
        base = 0.75
    elif macro_score > 0:
        base = 0.60
...
def apply_nav_breaker(base_exposure, dd_now, hard_mode):
    if hard_mode:
        if dd_now >= REENTRY_DD:
            ...
            return NAV_HARD_EXPOSURE_CAP, True, "NAV_HARD"
    if dd_now <= NAV_HARD_DD:
        return NAV_HARD_EXPOSURE_CAP, True, "NAV_HARD"
    elif dd_now <= NAV_MEDIUM_DD:
        return min(base_exposure, NAV_MEDIUM_EXPOSURE_CAP), False, "NAV_MEDIUM"
    elif dd_now <= NAV_SOFT_DD:
        return min(base_exposure, NAV_SOFT_EXPOSURE_CAP), False, "NAV_SOFT"
    else:
...
def build_signals_for_next_day(day_df, holdings, nav_basis, dd_now, hard_mode):
    ...
    for sym, pos in list(holdings.items()):
        ...
        if risk_mode == "NORMAL" and market_regime == 1:
    ...
    allow_new_positions = (market_regime == 1 and risk_mode == "NORMAL") or (market_regime == 0 and risk_mode not in ["NAV_HARD", "NAV_MEDIUM"])
...
def main():
    ...
    if (nav["nav"] <= 0).any():
        raise ValueError("åµæ¸¬å° nav <= 0ï¼è«æª¢æ¥ engine")
    if (nav["count"] > TOP_N).any():
        raise ValueError(f"åµæ¸¬å° count > TOP_N({TOP_N})ï¼è«æª¢æ¥ engine")
