...
def write_both(df, name):
    if name in ["final_action_plan.csv", "trade_plan.csv", "position_overlay.csv", "ignition_candidates.csv", "strategy_evolution.csv"]:
        try:
            ...
            print("v266.54 field lock warning:", name, e)
...
def set_action(df, buy, test, watch, buy_sub, test_sub, watch_sub):
    # v266.54.2：所有 mask 都安全轉成 Series，避免 scalar False/True 造成 pandas setitem KeyError。
    ...
    for name in stale_files:
        for p in [ROOT / name, DATA_DIR / name]:
            try:
                if p.exists():
                    p.unlink()
                    print("v266.54 cleared stale output:", p)
            except Exception as e:
                print("v266.54 clear warning:", p, e)
...
def main():
    ...
    meta = {
        "generated_at": taipei_now_str(),
        "source": "v266_54_position_behavior_hint",
    ...
    # v266.54：策略層也輸出 summary fallback；真正最終日期仍由 yml 最後鎖定一次。
    final_summary = {
        **meta,
        "source": "v266_54_position_behavior_hint",
