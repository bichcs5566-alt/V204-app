...
PRICE_PANEL_FILE = Path("price_panel_daily.csv")
...
def load_price():
    if not PRICE_PANEL_FILE.exists():
        raise FileNotFoundError("缺少 price_panel_daily.csv")
...
def build_trade_plan(df):
    ...
    def tier(px):
        if px < 50: return "50以下"
        if px < 100: return "50-100"
        if px < 300: return "100-300"
        if px < 500: return "300-500"
        if px < 1000: return "500-1000"
        return "1000以上"
    ...
    cols = ["signal_date", "trade_date", "action", "stock_id", "price_tier", "target_weight", "ref_price", "suggested_amount", "note"]
...
def main():
    ...
    empty_pos = pd.DataFrame(columns=["signal_date", "trade_date", "stock_id", "price_tier", "ref_price", "shares", "avg_cost", "pnl_pct", "target_weight", "current_weight_est", "action", "note"])
    empty_watch = pd.DataFrame(columns=["signal_date", "trade_date", "stock_id", "price_tier", "ref_price", "holding_status", "strategy_bucket", "action", "pnl_pct"])
    summary = pd.DataFrame([{"return": 0, "mdd": 0, "sharpe_daily": 0}])

    meta = {
        "generated_at": now_taipei().strftime("%Y-%m-%d %H:%M:%S"),
        "now_time": now_taipei().strftime("%Y-%m-%d %H:%M:%S"),
