...
# 升級：
# 1. 放寬 entry，增加交易頻率
# 2. 加入 breakout（突破近5日高點）
# 3. strict 模式提高曝險
# 4. 保留 risk_on / 止損 / 高集中
# 5. 輸出 nav / summary / trades / trade_stats
...
# Breakout：突破前5日高點（避免偷看）
df["high_5_prev"] = g["close"].rolling(5).max().shift(1).reset_index(level=0, drop=True)
df["breakout_5"] = df["close"] > df["high_5_prev"]

# 市場 risk_on：較進攻，但仍有過濾
market = (
...
def select_A(sub: pd.DataFrame):
    ...
    # v226.1：放寬 entry + breakout 優先
    strict = base[
        (base["mom_3"] > 0.02) &
        (base["mom_5"] > 0.03) &
        (base["mom_10"].fillna(0) > 0.00) &
        (base["std_3"] < 0.15)
    ].copy()

    if not strict.empty:
        strict["breakout_bonus"] = strict["breakout_5"].fillna(False).astype(int) * 0.08
        strict["score"] = (
            strict["mom_3"].fillna(0) * 0.45 +
            strict["mom_5"].fillna(0) * 0.30 +
            strict["mom_10"].fillna(0) * 0.10 +
            strict["breakout_bonus"] -
            strict["std_3"].fillna(0.2) * 0.10
        )
    ...
    # fallback 1：排名版
    ranked = base.copy()
    ranked["breakout_bonus"] = ranked["breakout_5"].fillna(False).astype(int) * 0.08
    ranked["score"] = (
        ranked["mom_3"].fillna(0) * 0.50 +
        ranked["mom_5"].fillna(0) * 0.25 +
        ranked["mom_10"].fillna(0) * 0.10 +
        ranked["breakout_bonus"] -
    ...
    # fallback 2：最少抓 2 檔
    min2 = base.sort_values(["mom_3", "mom_5"], ascending=False).head(min(2, len(base))).copy()
    if not min2.empty:
        min2["score"] = np.arange(len(min2), 0, -1)
...
for d in dates:
    ...
    # 保留風控，但略放寬以保留攻擊性
    picks["ret_1d"] = picks["ret_1d"].clip(lower=-0.08)
    ...
    keep_cols = [
        ...
        "breakout_5", "score", "ret_1d",
        "weight_engine", "weight_portfolio", "wret",
        "select_mode", "capital_after_trade"
    ]
    for c in keep_cols:
...
if not trades_df.empty:
    trade_stats = pd.DataFrame([{
        ...
        "breakout_rows": int(trades_df["breakout_5"].fillna(False).sum()),
    }])
else:
    trade_stats = pd.DataFrame([{
        "rows": 0,
...
nav_df.to_csv("v226_1_A_nav.csv", index=False)
trades_df.to_csv("v226_1_A_trades.csv", index=False)
summary.to_csv("v226_1_A_summary.csv", index=False)
trade_stats.to_csv("v226_1_A_trade_stats.csv", index=False)

print("✅ DONE V226.1 A ENGINE")
