...
def main():
    ...
    for r in rows:
        ...
        if sid == stock_id:
            ...
            r["shares"]=shares; r["avg_cost"]=avg_cost; r["note"]="v266.10.2 çåå¯«æ´æ°"
        new_rows.append({"stock_id":sid,"shares":str(r.get("shares","")).strip(),"avg_cost":str(r.get("avg_cost","")).strip(),"last_action_date":str(r.get("last_action_date","")).strip(),"note":str(r.get("note","")).strip()})
    if action_type == "upsert" and not found:
        new_rows.append({"stock_id":stock_id,"shares":shares,"avg_cost":avg_cost,"last_action_date":"","note":"v266.10.2 çåå¯«æ°å¢"})
    new_rows = sorted(new_rows, key=lambda x: x["stock_id"])
    write_rows(ROOT_POS, new_rows); write_rows(DASH_POS, new_rows)
    ...
    if diff.returncode != 0:
        subprocess.run(["git","commit","-m",f"position writeback {action_type} {stock_id}"], check=True); push_retry()
