"""
v266_2_daily_append.py
v266.2 每日更新：只 append 今日資料到完整母檔

用途：
- 不砍歷史資料
- 更新 price_panel_daily.csv
...
def parse_rows(payload, date_str, market):
    for fields, rows, source_key in candidate_tables(payload):
        joined = " | ".join(fields)
        if not ("收盤" in joined and ("證券代號" in joined or "代號" in joined)):
            continue
...
def fetch_twse_today():
    ...
    return parse_rows(resp.json(), TODAY, "TWSE")
...
def load_existing():
    ...
    if not p.exists() or p.stat().st_size == 0:
        raise FileNotFoundError("price_panel_daily.csv missing. Run chunk build + merge first.")
...
def main():
    ...
    if TODAY_DT.weekday() >= 5:
        print("weekend, keep existing data unchanged")
        panel = finalize(old)
    else:
        today = fetch_twse_today()
        if today.empty:
            print("today data empty, keep existing unchanged")
            panel = finalize(old)
        else:
    ...
    print("v266.2 daily append completed")
