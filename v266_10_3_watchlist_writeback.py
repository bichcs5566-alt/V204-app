...
def main():
    ...
    if action_type=="upsert" and not exists: out.append({"stock_id":stock_id,"note":"v266.10.3 èªé¸è¡æ°å¢"})
    ...
    if diff.returncode!=0:
        subprocess.run(["git","commit","-m",f"v266.10.3 watchlist {action_type} {stock_id}"],check=True); push_retry()
