...
def run_engine_a(day: pd.DataFrame):
    ...
    d = d[(d["mom3"] > 0.015) & (d["mom5"] > 0.02) & (d["mom10"].fillna(0) > -0.02)]

    if d.empty:
        return 0.0, 0, 0.0

    d["score"] = (
        d["mom3"].fillna(0) * 0.50
        + d["mom5"].fillna(0) * 0.30
        + d["mom10"].fillna(0) * 0.10
        - d["vol5"].fillna(0.2) * 0.10
    )
    d = d.sort_values("score", ascending=False).head(4).copy()
...
def run_engine_b(day: pd.DataFrame):
    ...
    d["score"] = (
        d["mom10"].fillna(0) * 0.55
        + d["mom5"].fillna(0) * 0.25
        + d["mom20"].fillna(0) * 0.10
        - d["vol5"].fillna(0.2) * 0.10
    )

    d = d.sort_values("score", ascending=False).head(8).copy()
    d = d[(d["mom10"] > -0.05) & (d["mom5"] > -0.05)]
...
def backtest_portfolio(df: pd.DataFrame):
    ...
    for d in sorted(df["trade_date"].dropna().unique()):
        ...
        if regime == "strong":
            target_w_a, target_w_b = 0.70, 0.60
        elif regime == "normal":
            target_w_a, target_w_b = 0.30, 0.55
        else:
            target_w_a, target_w_b = 0.05, 0.40
        ...
        w_a = prev_w_a * 0.5 + target_w_a * 0.5
        w_b = prev_w_b * 0.5 + target_w_b * 0.5
...
def write_reports(nav_df: pd.DataFrame, raw_df: pd.DataFrame):
    ...
    nav_df.to_csv("v230_1_nav.csv", index=False)
    summary.to_csv("v230_1_summary.csv", index=False)
    diagnostics.to_csv("v230_1_diagnostics.csv", index=False)
