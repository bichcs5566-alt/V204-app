# v256_enhanced_dual_engine.txt
# 覆蓋後改名：v256_enhanced_dual_engine.py
# v256 完整版：
# 1) 雙軌系統：Core 75% + Alpha 25%
# 2) 嚴格 T+1，不偷看
# 3) 提升整體曝險，降低過度保守
# 4) Alpha 改為「爆量 + 短動能 + 突破」
# 5) 放寬 RISK_OFF，減少過度躲避市場
# 6) 保留 NAV breaker 與停損分層
...
# Alpha sleeve（攻擊）
ALPHA_AGG_TOP_N = 4
ALPHA_AGG_EXPOSURE = 0.65
ALPHA_MAX_POSITION_WEIGHT = 0.26
ALPHA_STOP1 = -0.07
ALPHA_STOP2 = -0.12
...
def load_price_panel():
    ...
    df["vol_turnover20"] = g["turnover"].rolling(20, min_periods=20).mean().reset_index(level=0, drop=True)
    df["volume_spike"] = df["turnover"] / (df["vol_turnover20"] + 1e-9)
...
def rank_core(day_df, mode):
    ...
    if mode == "AGG":
        ...
        d["score"] = (d["mom20"] / (d["vol20"] + 1e-9)) * 0.35 + d["mom10"] * 0.40 + d["mom5"] * 0.25
    elif mode == "DEF":
...
def rank_alpha(day_df, mode):
    d = day_df.dropna(subset=["mom3", "mom5", "mom10", "vol20", "volume_spike"]).copy()
    d = d[d["turnover"].fillna(0) >= MIN_DAILY_TURNOVER].copy()
    d = d[d["close"] >= 12].copy()
    if mode != "AGG":
        return d.iloc[0:0].copy()
    d = d[(d["mom3"] > 0) & (d["mom5"] > 0) & (d["volume_spike"] > 1.5)].copy()
    d["score"] = d["mom3"] * 0.40 + d["mom5"] * 0.30 + d["breakout"] * 0.30
    return d.sort_values("score", ascending=False).reset_index(drop=True)
...
def build_core_packet(day_df, holdings, nav_basis, dd_now, hard_mode):
    ...
    if market_state == "AGG":
        base = 0.55 if macro_score > 0.5 else (0.48 if macro_score > 0 else 0.40)
...
def simulate(df):
    ...
    for date in all_dates:
        ...
        nav_rows.append([date, total_nav, core_nav, alpha_nav, core_cash, alpha_cash, total_exp, core_exp, alpha_exp, dd, core_count + alpha_count, market_state])
    ...
    nav_df = pd.DataFrame(nav_rows, columns=["date", "nav", "core_nav", "alpha_nav", "core_cash", "alpha_cash", "exposure", "core_exposure", "alpha_exposure", "dd", "count", "market_state"])
    nav_df["ret"] = nav_df["nav"].pct_change().fillna(0.0)
    trade_df = pd.DataFrame(trade_rows, columns=["date", "sleeve", "action", "symbol", "close_px", "exec_px", "shares", "trade_value", "fee_tax", "signal_nav_basis", "mode"])
    signal_df = pd.DataFrame(signal_rows, columns=["date", "sleeve", "signal", "symbol", "pnl_vs_cost", "target_exposure", "risk_mode", "mode"])
