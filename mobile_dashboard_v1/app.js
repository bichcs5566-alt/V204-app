...
// ===== v266.25.2 追加：持倉提示 overlay 接入（只補充，不改原本邏輯） =====
// 讀取 mobile_dashboard_v1/data/position_overlay.csv，並補到每張持倉卡片內。
// 若檔案不存在或 0 筆，不影響原本持倉畫面。
window.__positionOverlayMapV266252 = window.__positionOverlayMapV266252 || {};

function normalizePositionOverlayStockIdV266252(v) {
  const m = String(v || "").match(/(\d{4})/);
  return m ? m[1] : "";
}

function overlayActionClassV266252(action, riskFlag) {
  const a = String(action || "");
  const r = String(riskFlag || "").toUpperCase();
  if (a.includes("出場") || r.includes("STOP")) return "exit";
  if (a.includes("觀察") || r.includes("MA20") || r.includes("DATA") || r.includes("NO_")) return "watch";
  if (a.includes("抱住") || r.includes("HOLD")) return "hold";
  return "watch";
}

async function loadPositionOverlayV266252() {
  try {
    const txt = await fetchText("./data/position_overlay.csv");
    const rows = parseCsv(txt);
    const map = {};
    rows.forEach(r => {
      const sid = normalizePositionOverlayStockIdV266252(r.stock_id);
      if (sid) map[sid] = r;
    });
    window.__positionOverlayMapV266252 = map;
    renderPositionOverlayIntoCardsV266252();
  } catch (e) {
    console.log("position overlay load skipped", e);
    window.__positionOverlayMapV266252 = {};
  }
}

function renderPositionOverlayHTMLV266252(row) {
  row = safeObj(row);
  const action = safeText(row.position_action, "🟡 觀察");
  const reason = safeText(row.position_reason, "尚無持倉提示");
  const hint = safeText(row.position_hint, "持倉提示尚未產生。");
  const risk = safeText(row.risk_flag, "--");
  const close = safeText(row.close, "--");
  const ma20 = safeText(row.ma20, "--");
  const pnl = safeText(row.pnl_pct, "--");
  const cls = overlayActionClassV266252(action, risk);

  return `
    <div class="position-overlay-v266252 ${cls}">
      <div class="position-overlay-head-v266252">
        <span>持倉提示</span>
        <b>${action}</b>
      </div>
      <div class="position-overlay-grid-v266252">
        <div><span>原因</span><b>${reason}</b></div>
        <div><span>風險</span><b>${risk}</b></div>
        <div><span>現價</span><b>${close}</b></div>
        <div><span>MA20</span><b>${ma20}</b></div>
        <div><span>損益%</span><b>${pnl}</b></div>
      </div>
      <div class="position-overlay-hint-v266252">${hint}</div>
    </div>
  `;
}

function renderPositionOverlayIntoCardsV266252() {
  const map = window.__positionOverlayMapV266252 || {};
  const cards = document.querySelectorAll(".scan-item.position");
  if (!cards.length) return;

  cards.forEach(card => {
    const stockEl = card.querySelector(".scan-stock");
    const sid = normalizePositionOverlayStockIdV266252(stockEl ? stockEl.textContent : "");
    const detail = card.querySelector(".scan-detail");
    if (!sid || !detail) return;

    const old = detail.querySelector(".position-overlay-v266252");
    if (old) old.remove();
    ...
    const wrap = document.createElement("div");
    wrap.innerHTML = renderPositionOverlayHTMLV266252(row);
    const node = wrap.firstElementChild;
...
// 包一層原本 renderPositions，不改原本內容，只在後面補 overlay
if (typeof renderPositions === "function" && !window.__renderPositionsOverlayWrappedV266252) {
  window.__renderPositionsOverlayWrappedV266252 = true;
  const originalRenderPositionsV266252 = renderPositions;
  renderPositions = function() {
    originalRenderPositionsV266252.apply(this, arguments);
    setTimeout(renderPositionOverlayIntoCardsV266252, 0);
  };
}

// 等原本畫面完成後再載入 overlay
setTimeout(loadPositionOverlayV266252, 800);
setTimeout(renderPositionOverlayIntoCardsV266252, 1800);
setTimeout(renderPositionOverlayIntoCardsV266252, 3200);
