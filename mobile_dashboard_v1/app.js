...
function injectV381Style(){
  ...
  style.id="v381-style";
  style.textContent=`
    .decision-hint,.v39-pill{display:inline-block;padding:6px 10px;border-radius:999px;font-size:13px;font-weight:800;white-space:nowrap;background:#f2f4f7;color:#344054}
    .decision-strong,.v39-strong{background:#fff1f0;color:#b42318}
    .decision-warn,.v39-warn{background:#fff8e6;color:#b54708}
    .decision-danger,.v39-danger{background:#fdecec;color:#b42318}
    .decision-sync,.v39-sync{background:#eef3ff;color:#25406f}
    .v39-market-bull{background:#ecfdf3;color:#027a48}
    .v39-market-neutral{background:#fff8e6;color:#b54708}
    .v39-market-bear{background:#fdecec;color:#b42318}
    .v39-lots{font-weight:900;color:#172554;white-space:nowrap}
  `;
...
function renderTradePlan(rows){const body=document.getElementById("tradePlanBody");if(!rows.length){body.innerHTML=`<tr><td colspan="11" class="empty">目前沒有資料</td></tr>`;return}body.innerHTML=rows.map(r=>`<tr><td>${badgeForAction(r.action)}</td><td>${safe(r.stock_id)}</td><td>${prettyTier(r.price_tier)}</td><td>${safe(r.ref_price)}</td><td>${safe(r.target_weight)}</td><td>${safeMoney(r.suggested_amount)}</td><td>${decisionHintBadge(getDecisionHint(r.action,r.chip_label))}</td><td>${v39DecisionBadge(r)}</td><td>${v39LotsBadge(r)}</td><td>${v39MarketBadge(r)}</td><td>${safe(r.note)}</td></tr>`).join("")}

function renderPosition(rows){
  const body=document.getElementById("positionBody");
  if(!rows.length){body.innerHTML=`<tr><td colspan="14" class="empty">目前沒有持倉資料</td></tr>`;return}
...
// ================================
// v3.9.1 UI Execution Layer
// 不改策略、不改寫回、不改 CSV，只新增：決策 / 張數 / 市場顯示
// ================================
...
function getActionRatioV39(action){
  ...
  if(action==="STOP_LOSS")return 1.0;
  return 0;
}
...
function calcLotsV39(r){
  ...
  const price=toNum(r.ref_price)||toNum(r.avg_cost);
  ...
  if(action==="REDUCE"||action==="SELL"||action==="STOP_LOSS"){
    if(!shares||shares<lotSize)return "--";
...
function getMarketV39(r){
  ...
  if(mom20<0)return "🔴 空頭";
  return "⚪ 未判定";
}

function getDecisionV39(r){
  const action=(r.action||"").toUpperCase();
  const chip=String(r.chip_label||"");
  const strong=chip.includes("強")||chip.includes("偏強")||chip.includes("加分");
  const warn=chip.includes("注意")||chip.includes("⚠️");

  if(action==="BUY"){
    if(strong)return "🔥 建倉100%";
    if(warn)return "⚠️ 小倉或略過";
    return "👉 建倉";
  }
  if(action==="ADD"){
    if(strong)return "🚀 加碼50%";
    if(warn)return "⚠️ 暫緩加碼";
    return "👉 小幅加碼";
  }
  if(action==="REDUCE"){
    if(strong)return "⚠️ 減碼30%";
    if(warn)return "❗ 風險減碼";
    return "👉 減碼";
  }
...
function v39DecisionBadge(r){
  const decision=getDecisionV39(r);
  let cls="v39-pill";
  if(decision.includes("🔥")||decision.includes("🚀"))cls+=" v39-strong";
  else if(decision.includes("⚠️"))cls+=" v39-warn";
  else if(decision.includes("❗"))cls+=" v39-danger";
  else if(decision.includes("⏳"))cls+=" v39-sync";
  return `<span class="${cls}">${safe(decision)}</span>`;
}

function v39LotsBadge(r){
  const lots=calcLotsV39(r);
  return `<span class="v39-lots">${safe(lots)}${lots==="--"?"":" 張"}</span>`;
}

function v39MarketBadge(r){
  const market=getMarketV39(r);
  let cls="v39-pill";
  if(market.includes("多頭"))cls+=" v39-market-bull";
  else if(market.includes("中性"))cls+=" v39-market-neutral";
  else if(market.includes("空頭"))cls+=" v39-market-bear";
  return `<span class="${cls}">${safe(market)}</span>`;
}
