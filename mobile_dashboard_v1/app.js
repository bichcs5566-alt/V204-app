const DATA_DIR="./data";const GH_CONFIG_KEY="v3_github_config";
...
function bindUI(){
  ...
  bind("addPositionBtn","click",submitAddPosition);
  document.addEventListener("click",async e=>{const b=e.target.closest("[data-remove-position]");if(b)await submitRemovePosition(b.getAttribute("data-remove-position"));});
}
...
async function dispatchWorkflow(workflowId,inputs={},successMessage="已送出"){const cfg=getGithubConfig();if(!cfg)return false;const url=`https://api.github.com/repos/${cfg.owner}/${cfg.repo}/actions/workflows/${workflowId}/dispatches`;const res=await fetch(url,{method:"POST",headers:{"Accept":"application/vnd.github+json","Authorization":`Bearer ${cfg.token}`,"Content-Type":"application/json","X-GitHub-Api-Version":"2022-11-28"},body:JSON.stringify({ref:cfg.branch,inputs})});if(!res.ok){const txt=await res.text();setBanner(`送出 workflow 失敗：${res.status}`,"#b42318");console.error(txt);return false}setBanner(successMessage,"#2f7d32");return true}

async function submitAddPosition(){const stockId=val("positionStockInput").trim(),shares=val("positionSharesInput").trim(),avgCost=val("positionCostInput").trim();if(!stockId||!shares||!avgCost){setBanner("加入持倉前請填完整：股票代號 / 股數 / 成本","#b42318");return}const ok=await dispatchWorkflow("v3_position_writeback.yml",{action:"add",stock_id:stockId,shares:shares,avg_cost:avgCost},`已送出持倉新增 / 更新：${stockId}，請等待 Actions 跑完後重新整理。`);if(ok){val("positionStockInput","");val("positionSharesInput","");val("positionCostInput","")}}

async function submitRemovePosition(stockId){stockId=String(stockId||"").trim();if(!stockId){setBanner("找不到要移除的股票代號","#b42318");return}if(!confirm(`確定要移除持倉 ${stockId} 嗎？`))return;await dispatchWorkflow("v3_position_writeback.yml",{action:"remove",stock_id:stockId,shares:"",avg_cost:""},`已送出持倉移除：${stockId}，請等待 Actions 跑完後重新整理。`)}

async function loadAll(force=false){try{const[meta,tradePlan,position,summary,debug]=await Promise.all([fetchJSON(`${DATA_DIR}/meta.json`,force),fetchCSV(`${DATA_DIR}/trade_plan.csv`,force),fetchCSV(`${DATA_DIR}/position_monitor.csv`,force),fetchCSV(`${DATA_DIR}/full_summary.csv`,force),fetchCSV(`${DATA_DIR}/selection_debug.csv`,force)]);renderMeta(meta);renderTradePlan(tradePlan);renderPosition(position);renderSummary(summary);renderDebug(debug);renderTierSummary(position);renderActionSummary(tradePlan,position);if(!document.getElementById("syncBanner").textContent.includes("已送出"))setBanner("頁面資料已同步","#2f7d32")}catch(err){console.error(err);setBanner(`讀取失敗：${err.message}`,"#b42318")}}
...
function renderTierSummary(positionRows){const tiers={};positionRows.forEach(r=>{const key=prettyTier(r.price_tier||"unknown");tiers[key]=(tiers[key]||0)+1});const container=document.getElementById("tierSummary"),entries=Object.entries(tiers);if(!entries.length){container.innerHTML=`<div class="tier-box"><div class="tier-label">分層狀態</div><div class="tier-value">--</div><div class="tier-sub">目前沒有持倉可展示</div></div>`;return}container.innerHTML=entries.map(([k,v])=>`<div class="tier-box"><div class="tier-label">${k}</div><div class="tier-value">${v}</div><div class="tier-sub">此分層目前有 ${v} 檔持倉</div></div>`).join("")}

function renderActionSummary(tradeRows,positionRows){const buys=tradeRows.filter(r=>(r.action||"").toUpperCase()==="BUY");const positionActions=positionRows.filter(r=>["ADD","REDUCE","SELL","STOP_LOSS"].includes((r.action||"").toUpperCase()));let headline="觀察",desc="今天沒有新的買進動作";if(buys.length>0){headline="偏多";desc=`今天有 ${buys.length} 檔新進場候選`}else if(positionActions.length>0){headline="調整";desc=`今天有 ${positionActions.length} 筆持倉調整`}const totalBuyAmount=buys.reduce((acc,r)=>acc+toNum(r.suggested_amount),0);const stopLossCount=positionRows.filter(r=>(r.action||"").toUpperCase()==="STOP_LOSS").length;text("headlineAction",headline);text("headlineDesc",desc);text("buyCount",String(buys.length));text("buyAmount",`建議金額：${moneyDisplay(totalBuyAmount)}`);text("positionActionCount",String(positionActions.length));text("positionActionDesc",positionActions.length?"請優先查看持倉監控區":"目前無加減碼");text("riskLevel",stopLossCount>0?"偏高":buys.length>8?"中等":"正常");text("riskDesc",stopLossCount>0?`有 ${stopLossCount} 筆停損訊號`:"目前沒有明顯停損警示")}

function badgeForAction(actionRaw){const action=(actionRaw||"").toUpperCase();const map={BUY:["badge-buy","買進"],HOLD:["badge-hold","持有"],SELL:["badge-sell","賣出"],ADD:["badge-add","加碼"],REDUCE:["badge-reduce","減碼"],STOP_LOSS:["badge-stop","停損"]};const pair=map[action]||["badge-hold",safe(actionRaw)];return`<span class="badge ${pair[0]}">${pair[1]}</span>`}
