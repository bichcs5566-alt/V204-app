...
/* =========================================================
   v331 Position Detail Same-Field Renderer / 持倉展開欄位一致化
   目的：
   - 只調整「持倉卡片展開內容」
   - 不回寫持倉、不動 Actions、不動後端策略、不動其他清單 renderer
   - 持倉展開欄位改成跟其他清單一樣的 detail-grid 欄位卡片
   - 保留持倉必要內容：成本、現價、損益%、張數、股數、市值、持倉判斷、操作建議
   ========================================================= */

function posPickV331(row, keys, fallback = "") {
  row = row || {};
  for (const k of keys) {
    const v = row[k];
    if (v !== undefined && v !== null && String(v).trim() !== "" && !["--", "nan", "NaN", "undefined", "null", "None"].includes(String(v).trim())) {
      return v;
    }
  }
  return fallback;
...
function posNumberV331(v) {
  const n = Number(String(v ?? "").replace(/,/g, "").replace("%", "").replace("張", "").replace("股", ""));
  return Number.isFinite(n) ? n : null;
}

function posPriceV331(v) {
  const n = posNumberV331(v);
  if (n === null) return "--";
  return Number.isInteger(n) ? String(n) : n.toFixed(2).replace(/\.00$/, "");
}

function posMoneyV331(v) {
  const n = posNumberV331(v);
  if (n === null) return "--";
  return Math.round(n).toLocaleString("zh-TW");
}

function posPctV331(v) {
  const n = posNumberV331(v);
  if (n === null) return "--";
  return n.toFixed(2).replace(/\.00$/, "") + "%";
}

function posActionInfoV331(raw, pnlPct, listName) {
  const s = String(raw || "").toUpperCase();
  const list = String(listName || "").toUpperCase();

  if (/BLOCK|禁止/.test(list)) return { key: "BLOCK", cls: "block", label: "⛔ 禁止加碼", text: "禁止加碼" };
  if (/SELL|STOP|出場|賣/.test(s)) return { key: "SELL", cls: "sell", label: "🔴 出場", text: "出場" };
  if (/REDUCE|減碼|減/.test(s)) return { key: "REDUCE", cls: "reduce", label: "🟠 減碼", text: "減碼" };
  if (/BUY|買進|PRIORITY|CORE|EVOLUTION-A/.test(s + " " + list)) return { key: "BUY", cls: "buy", label: "🟢 續抱 / 可加碼", text: "續抱 / 可加碼" };
  if (/TEST|試單|IGNITION/.test(s + " " + list)) return { key: "TEST", cls: "test", label: "🟡 續抱觀察", text: "續抱觀察" };
  if (Number.isFinite(pnlPct) && pnlPct <= -8) return { key: "REDUCE", cls: "reduce", label: "🟠 風控檢查", text: "風控檢查" };
  return { key: "HOLD", cls: "watch", label: "🟢 續抱觀察", text: "續抱觀察" };
}

function posResolveListNameV331(stock, merged) {
  const sid = typeof stockKeyV26630H === "function" ? stockKeyV26630H(stock) : String(stock || "").trim();
  const row = merged || {};

  const direct = posPickV331(row, [
    "evolution_stage_v329",
    "strategy_bucket",
    "strategy_layer",
    "bucket",
    "strategy_type",
  ...
  if (/BLOCK|禁止/.test(joined)) return "BLOCK 禁止清單";
  if (/FINAL|PRIORITY|主升操作池|BUY|買進/.test(joined)) return "最終操作 / 主升操作池";
  if (/EVOLUTION-A/.test(joined)) return "EVOLUTION-A 準主升";
  if (/EVOLUTION-B/.test(joined)) return "EVOLUTION-B 培養觀察";
  if (/EVOLUTION/.test(joined)) return "EVOLUTION 策略進化";
  if (/IGNITION|起漲/.test(joined)) return "IGNITION 起漲訊號";
  if (/TEST|試單/.test(joined)) return "TEST 試單清單";
  if (/WATCH|觀察/.test(joined)) return "WATCH 觀察清單";

  try {
    const t = window.__techMapV26637?.[sid] || {};
    const j2 = Object.values(t).map(v => String(v ?? "")).join(" ").toUpperCase();
    if (/PRIORITY|BUY|買進/.test(j2)) return "最終操作 / 主升操作池";
    if (/EVOLUTION-A/.test(j2)) return "EVOLUTION-A 準主升";
    if (/EVOLUTION-B/.test(j2)) return "EVOLUTION-B 培養觀察";
    if (/EVOLUTION/.test(j2)) return "EVOLUTION 策略進化";
    if (/IGNITION/.test(j2)) return "IGNITION 起漲訊號";
    if (/TEST/.test(j2)) return "TEST 試單清單";
    if (/WATCH/.test(j2)) return "WATCH 觀察清單";
    if (/BLOCK/.test(j2)) return "BLOCK 禁止清單";
  } catch (e) {}

  return "不在目前清單";
}

function posStrategyLayerV331(row, listName) {
  const direct = posPickV331(row, ["evolution_stage_v329", "strategy_bucket", "strategy_layer", "layer", "strategy_type", "bucket"], "");
  if (direct) return cleanV26630 ? cleanV26630(direct, "--") : String(direct);
  if (/最終操作|PRIORITY/.test(listName)) return "PRIORITY / CORE 主升";
  if (/EVOLUTION-A/.test(listName)) return "EVOLUTION-A 準主升";
  if (/EVOLUTION-B/.test(listName)) return "EVOLUTION-B 主升培養";
  if (/IGNITION/.test(listName)) return "IGNITION 起漲";
  if (/TEST/.test(listName)) return "TEST 試單";
  if (/WATCH/.test(listName)) return "WATCH 觀察";
  if (/BLOCK/.test(listName)) return "BLOCK 禁止";
  return "持倉管理";
}

function posAdviceV331(actionInfo, listName, pnlPct, ma5, ma20) {
  if (actionInfo.key === "BLOCK") return "此股目前落在禁止或高風險區，不建議加碼；若已持有，優先檢查停損與部位大小。";
  if (actionInfo.key === "SELL") return "持倉已有出場或停損訊號，優先處理風險，不建議拖延。";
  if (actionInfo.key === "REDUCE") return "先降低部位風險，等待站回關鍵均線或重新進入強勢清單再評估。";
  if (/最終操作|EVOLUTION-A/.test(listName)) return "此股仍在主升候選或最終操作邏輯內，可續抱；若要加碼，以分批、低於原本風險上限為主。";
  if (/EVOLUTION-B|IGNITION|TEST/.test(listName)) return "仍屬培養或試單層，建議續抱觀察，不要單筆重倉；等待升級 A 或進最終操作再提高權重。";
  if (/WATCH/.test(listName)) return "目前只屬觀察層，持倉可小心續抱，但不建議新增加碼。";
  if (Number.isFinite(pnlPct) && pnlPct < 0) return "目前未進主要清單且損益為負，應降低期待，依 MA5 / MA20 與停損線控風險。";
  return "目前以本機持倉重新評估；若未在主清單內，僅作續抱觀察，不主動加碼。";
}

function renderMergedPositionHintV26630(stock, posRow) {
  const sid = typeof stockKeyV26630H === "function" ? stockKeyV26630H(stock) : String(stock || "").trim();
  const overlay = { ...(window.__techMapV26637?.[sid] || {}), ...(typeof getPositionOverlayRowV26630 === "function" ? getPositionOverlayRowV26630(sid) : {}) };
  const riskRow = { ...(window.__techMapV26637?.[sid] || {}), ...(typeof getPositionRiskRowV26630 === "function" ? getPositionRiskRowV26630(sid) : {}) };
  const merged = { ...overlay, ...riskRow, ...posRow, stock_id: sid };

  const name = typeof positionNameV26630 === "function" ? positionNameV26630(sid, posRow, overlay, riskRow) : posPickV331(merged, ["stock_name", "name"], "--");
  const listName = posResolveListNameV331(sid, merged);
  const strategyLayer = posStrategyLayerV331(merged, listName);

  const avgNum = posNumberV331(posRow.avg_price);
  const sharesNum = posNumberV331(posRow.shares);
  const lotsNum = posNumberV331(posRow.lots);
  const closeRaw = posPickV331(merged, ["close", "ref_price", "price", "last_price", "current_price"], posRow.avg_price);
  const closeNum = posNumberV331(closeRaw);
  const pnlPctNum = (Number.isFinite(closeNum) && Number.isFinite(avgNum) && avgNum > 0)
    ? ((closeNum - avgNum) / avgNum * 100)
    : null;

  const avg = posPriceV331(posRow.avg_price);
  const close = posPriceV331(closeRaw);
  const shares = Number.isFinite(sharesNum) ? Math.round(sharesNum).toLocaleString("zh-TW") + "股" : "--";
  const lots = Number.isFinite(lotsNum) ? String(lotsNum).replace(/\.00$/, "") + "張" : "--";
  const cost = (Number.isFinite(avgNum) && Number.isFinite(sharesNum)) ? posMoneyV331(avgNum * sharesNum) : "--";
  const marketValue = (Number.isFinite(closeNum) && Number.isFinite(sharesNum)) ? posMoneyV331(closeNum * sharesNum) : "--";
  const pnlPct = Number.isFinite(pnlPctNum) ? posPctV331(pnlPctNum) : "--";
  const pnlAmount = (Number.isFinite(closeNum) && Number.isFinite(avgNum) && Number.isFinite(sharesNum))
    ? posMoneyV331((closeNum - avgNum) * sharesNum)
    : "--";

  const rawAction = posPickV331(merged, ["position_action", "final_action", "action", "status"], "HOLD");
  const actionInfo = posActionInfoV331(rawAction, pnlPctNum, listName);

  const liqLabel = typeof liquidityLabel === "function" ? liquidityLabel(merged.liquidity_level || merged.liquidity_tag) : "--";
  const liqCls = typeof liquidityClass === "function" ? liquidityClass(merged.liquidity_level || merged.liquidity_tag) : "";
  const liqScore = posPickV331(merged, ["liquidity_score"], "--");

  const ma5 = typeof inferMaLabelV26635 === "function" ? inferMaLabelV26635(merged, "ma5", "MA5") : "--";
  const ma10 = typeof inferMaLabelV26635 === "function" ? inferMaLabelV26635(merged, "ma10", "MA10") : "--";
  const ma20 = typeof inferMaLabelV26635 === "function" ? inferMaLabelV26635(merged, "ma20", "MA20") : "--";
  const kbar = typeof inferKbarTypeV26635 === "function" ? inferKbarTypeV26635(merged) : "--";
  const kstruct = typeof inferKStructureV26635 === "function" ? inferKStructureV26635(merged) : "--";
  const chip = typeof chipDisplayV26621 === "function" ? chipDisplayV26621(merged) : "--";
  const chipReason = typeof chipReasonV26621 === "function" ? chipReasonV26621(merged) : "籌碼依策略判斷";
  const chipHint = typeof chipHintV26621 === "function" ? chipHintV26621(merged) : "籌碼依策略判斷，只能當輔助。";
  const industry = typeof industryTagV3065 === "function" ? industryTagV3065(merged) : "--";

  const reason = posPickV331(merged, ["position_reason", "reason", "note"], "本機持倉重新評估：依最新策略清單、現價、均線、籌碼與風控狀態判斷。");
  const systemNote = posPickV331(merged, ["position_hint", "system_note", "action_sub"], "此持倉只存在本機，不回寫後端；重跑策略後會用最新清單重新評估。");
  const advice = posAdviceV331(actionInfo, listName, pnlPctNum, ma5, ma20);

  return `
    <div class="position-standard-v331 ${actionInfo.cls}">
      <div class="detail-grid position-standard-grid-v331">
        ${detailCell("股票名稱", name)}
        ${detailCell("今日主判斷", actionInfo.text)}
        ${detailCell("所在清單", listName)}
        ${detailCell("策略層", strategyLayer)}
        ${detailCell("產業類別", industry)}
        ${detailCell("參考價", close)}
        ${detailCell("成本價", avg)}
        ${detailCell("損益%", pnlPct)}
        ${detailCell("損益金額", pnlAmount)}
        ${detailCell("張數", lots)}
        ${detailCell("股數", shares)}
        ${detailCell("成本金額", cost)}
        ${detailCell("目前市值", marketValue)}
        ${detailCell("流動性", liqLabel, liqCls)}
        ${detailCell("流動性分數", liqScore)}
        ${detailCell("MA5觀察", ma5)}
        ${detailCell("MA10觀察", ma10)}
        ${detailCell("MA20觀察", ma20)}
        ${detailCell("K棒型態", kbar)}
        ${detailCell("K線結構", kstruct)}
        ${detailCell("籌碼集中度", chip)}
        ${detailCell("更新時間", cleanV26630 ? cleanV26630(posRow.updated_at) : (posRow.updated_at || "--"))}
        ${detailCell("備註", cleanV26630 ? cleanV26630(posRow.note, "手動持倉") : (posRow.note || "手動持倉"))}
      </div>

      <div class="detail-text position-standard-text-v331"><b>持倉原因</b><p>${reason}</p></div>
      <div class="detail-text position-standard-text-v331"><b>操作建議</b><p>${advice}</p></div>
      <div class="detail-text position-standard-text-v331"><b>技術補充</b><p>${ma5}｜${ma10}｜${ma20}｜K棒：${kbar}｜K線：${kstruct}</p></div>
      <div class="detail-text position-standard-text-v331"><b>籌碼提示</b><p>${chipReason}｜${chipHint}</p></div>
      <div class="detail-text position-standard-text-v331"><b>系統提示</b><p>${systemNote}</p></div>
    </div>
  `;
}

function injectPositionStandardStyleV331() {
  try {
    if (document.getElementById("position-standard-style-v331")) return;
    const style = document.createElement("style");
    style.id = "position-standard-style-v331";
    style.textContent = `
      .position-standard-v331 {
        margin-top: 0 !important;
        padding: 0 !important;
        background: transparent !important;
        border: 0 !important;
      }
      .position-standard-grid-v331 {
        margin-top: 0 !important;
      }
      .position-standard-text-v331 {
        margin-top: 12px !important;
      }
      .scan-item.position .scan-detail.open {
        padding-top: 16px !important;
      }
      .scan-item.position .detail-grid div {
        background: #f8fafc !important;
        border: 1px solid #e5e7eb !important;
      }
      .scan-item.position .detail-grid div b {
...
try { injectPositionStandardStyleV331(); } catch(e) {}
document.addEventListener("DOMContentLoaded", function() {
  try { injectPositionStandardStyleV331(); } catch(e) {}
});
