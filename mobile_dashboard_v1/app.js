...
/* =========================================================
   v266.76 FINAL NATIVE RENDER 2330 TEST PATCH
   修正重點：
   - 不手刻 FINAL 卡片
   - 直接使用原本 renderScanRow(row, key) 產生卡片
   - 所以卡片大小、欄位排列、展開/收合邏輯跟前面四大清單一致
   - 只塞 2330 測試資料到 FINAL
   - 不改策略、不改 CSV、不改 pipeline
   ========================================================= */

function injectFinalNativeDemoStyleV26676() {
  if (document.getElementById("final-native-demo-style-v26676")) return;

  const style = document.createElement("style");
  style.id = "final-native-demo-style-v26676";
  style.textContent = `
    .final-native-test-warning-v26676 {
      margin: 10px 0 14px;
      padding: 12px 14px;
      border-radius: 16px;
    ...
    .final-native-test-warning-v26676 small {
      display: block;
      margin-top: 4px;
      font-size: 12px;
      font-weight: 850;
    ...
    .final-section-native-v26676 {
      border: 2px solid rgba(239, 68, 68, .30) !important;
      background: linear-gradient(180deg, #fff7f7, #ffffff) !important;
      box-shadow: 0 0 0 2px rgba(239, 68, 68, .07) !important;
    }

    .final-section-native-v26676 h2,
    .final-section-native-v26676 summary {
      color: #b91c1c !important;
      font-weight: 950 !important;
    }

    .final-native-card-v26676.scan-item {
      border-color: #ef4444 !important;
      box-shadow: 0 0 0 2px rgba(239, 68, 68, .10) !important;
    }

    .final-native-card-v26676 .scan-main-live,
    .final-native-card-v26676 .scan-main {
      background: linear-gradient(90deg, #fff1f2, #ffffff) !important;
      border-color: rgba(239, 68, 68, .45) !important;
    }

    .final-native-card-v26676 .scan-action {
    ...
    .final-native-card-v26676 .scan-action::after {
      content: "";
    }

    .final-native-card-v26676 .hint,
    .final-native-card-v26676 .system-note {
      color: #991b1b !important;
...
function buildFinalDemoRow2330V26676() {
  return {
    stock_id: "2330",
    stock_name: "台積電",
    industry: "半導體",
    ...
    strategy_type: "FINAL",
    bucket: "FINAL",
    strategy_name: "FINAL 主升確認",
    source: "manual_final_ui_test_native_render",
    ref_price: "982.00",
    price: "982.00",
    close: "982.00",
    score: "92",
    ...
    chip_score: "88（🔥 高集中）",
    ma5_status: "MA5：站上｜↑ 強勢",
    ma10_status: "MA10：站上｜↑ 強勢",
    ma20_status: "MA20：站上｜↑ 強勢",
    kbar_type: "主升續強K",
    kline_structure: "多頭排列",
    progress_state: "主升確認",
    behavior_state: "主升趨勢延續",
    risk_level: "未過熱",
    reason: "FINAL：主力慢推後確認主升，量價健康，未進入過熱；此筆為 UI 測試資料。",
    system_note: "FINAL 測試：主升確認卡片。可觀察卡片大小是否與其他清單一致。",
    decision_hint: "可列入正式操作區；若已有持倉，優先評估續抱或分批加碼。若短線乖離過大、爆量急拉，不追價。",
    chinese_decision_hint: "正式操作區：主力拉升確認、量價健康、未過熱。"
  };
}

function applyFinalNativeDemo2330V26676() {
  try { injectFinalNativeDemoStyleV26676(); } catch (e) {}

  const finalList = document.getElementById("finalActionList");
  if (!finalList || typeof renderScanRow !== "function") return;

  const finalSection = finalList.closest("section");
  if (finalSection) {
    finalSection.classList.add("final-section-native-v26676");
    ...
    if (hint) hint.textContent = "主升確認區：此版本用原生卡片模板顯示 2330 測試資料。";
  }

  const row = buildFinalDemoRow2330V26676();
  let html = "";

  try {
    html = renderScanRow(row, "FINAL");
  } catch (e) {
    html = "";
  }
  ...
  if (!html) {
    finalList.innerHTML = `
      <div class="final-native-test-warning-v26676">
        ⚠️ FINAL UI 測試模式：renderScanRow 尚未可用
        <small>請重新整理或確認 app.js 載入順序。</small>
      </div>
    `;
    return;
  }

  html = html.replace('<article class="scan-item ', '<article class="scan-item final-native-card-v26676 ');
  html = html.replace(/<div class="scan-action [^"]*">[\s\S]*?<\/div>/, '<div class="scan-action">🔥 主升</div>');

  finalList.innerHTML = `
    <div class="final-native-test-warning-v26676">
      ⚠️ FINAL UI 測試模式：以下 2330 不是正式策略訊號
      <small>這張卡片使用原本 renderScanRow 產生，大小應與四大清單一致。</small>
    </div>
    ${html}
  `;
}

function applyFinalNativeDemoLoopV26676() {
  applyFinalNativeDemo2330V26676();
...
try { applyFinalNativeDemoLoopV26676(); } catch (e) {}
document.addEventListener("DOMContentLoaded", function() {
  try { applyFinalNativeDemoLoopV26676(); } catch (e) {}
});
