...
def build_features(df):
    ...
    def reg(r):
        return "strong" if r["ma5"] > r["ma10"] else "weak"
...
def engine_A(d):
    ...
    d["score"] = d["mom3"]*0.4 + d["mom5"]*0.3 + d["mom10"]*0.3
...
def get_exposure(price_regime):
    return 1.1 if price_regime == "strong" else 0.8
...
def backtest(df):
    ...
    for date in sorted(df["trade_date"].unique()):
        ...
        # ① NAV 連續走弱
        if len(nav_hist) > 5:
            if nav_hist[-1] < nav_hist[-3] < nav_hist[-5]:
                wA *= 0.8

        # ② 短期回撤
        if len(nav_hist) > 5:
            recent_dd = nav_hist[-1] / max(nav_hist[-5:]) - 1
            if recent_dd < -0.04:
                wA *= 0.8
                wB *= 0.9
        ...
        # ===== 原本 NAV 控制 =====
        if len(nav_hist) > 12:
            if np.mean(nav_hist[-5:]) < np.mean(nav_hist[-12:]) * 0.995:
                wA *= 0.85
...
def main():
    ...
    out.to_csv("v234_3_nav.csv", index=False)
    summary.to_csv("v234_3_summary.csv", index=False)
