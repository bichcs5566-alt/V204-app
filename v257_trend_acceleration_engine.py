# v257_trend_acceleration_engine.txt
# 覆蓋後改名：v257_trend_acceleration_engine.py
# v257 趨勢加速版：
# 1) 雙軌系統：Core 70% + Alpha 30%
# 2) Alpha 從短動能改成「主升段 + 加速」
# 3) 強制最低曝險，避免過度保守
# 4) 嚴格 T+1，不偷看
# 5) 仍保留 risk breaker 與 market state
...
CORE_DEF_TOP_N = 1
CORE_DEF_EXPOSURE = 0.05
CORE_RISK_OFF_EXPOSURE = 0.00
CORE_MAX_POSITION_WEIGHT = 0.16
CORE_STOP1 = -0.06
CORE_STOP2 = -0.10

# Alpha
ALPHA_AGG_TOP_N = 3
ALPHA_AGG_EXPOSURE = 0.85
ALPHA_MAX_POSITION_WEIGHT = 0.30
ALPHA_STOP1 = -0.07
ALPHA_STOP2 = -0.12

# Global minimum exposure in AGG market
MIN_TOTAL_EXPOSURE_AGG = 0.35
...
def load_price_panel():
    ...
    df["turnover20"] = g["turnover"].rolling(20, min_periods=20).mean().reset_index(level=0, drop=True)
    df["volume_spike"] = df["turnover"] / (df["turnover20"] + 1e-9)
    df["hh20_prev"] = g["close"].rolling(20, min_periods=20).max().shift(1).reset_index(level=0, drop=True)
    df["breakout"] = (df["close"] >= df["hh20_prev"]).astype(float)
    df["accel"] = df["mom5"] - df["mom20"]
    return df
...
def build_market_state(price):
    ...
    risk_off = (mkt["market_close"] < mkt["ma60"] * 0.94) | (mkt["market_ret_5d"] < -0.12)
    defensive = (mkt["market_close"] < mkt["ma20"]) | (mkt["market_ret_5d"] < -0.02)
...
def ensure_state_fields(state):
    defaults = {"shares": 0.0, "avg_cost": 0.0, "weight_mult": 1.0, "stop1_hit": False}
...
def rank_core(day_df, mode):
    ...
    if mode == "AGG":
        d = d[(d["mom10"] > -0.01) & (d["mom20"] > -0.03)].copy()
        d["score"] = (d["mom20"] / (d["vol20"] + 1e-9)) * 0.30 + d["mom10"] * 0.40 + d["mom5"] * 0.30
    elif mode == "DEF":
...
def rank_alpha(day_df, mode):
    d = day_df.dropna(subset=["mom5", "mom20", "mom60", "accel", "volume_spike"]).copy()
    d = d[d["turnover"].fillna(0) >= MIN_DAILY_TURNOVER].copy()
    d = d[d["close"] >= 12].copy()
    if mode != "AGG":
        return d.iloc[0:0].copy()
    ...
    # 主升段 + 加速
    d = d[(d["mom20"] > 0) & (d["mom60"] > 0) & (d["accel"] > 0) & (d["volume_spike"] > 1.2)].copy()
    d["trend_strength"] = d["mom20"] * 0.6 + d["mom60"] * 0.4
    d["score"] = d["trend_strength"] * 0.50 + d["accel"] * 0.25 + d["breakout"] * 0.15 + (d["volume_spike"].clip(upper=3) / 3.0) * 0.10
    return d.sort_values("score", ascending=False).reset_index(drop=True)
...
def build_core_packet(day_df, holdings, nav_basis, dd_now, hard_mode):
    ...
    if market_state == "AGG":
        base = 0.50 if macro_score > 0.5 else (0.45 if macro_score > 0 else 0.38)
        top_n, max_pos, stop1, stop2, mode = CORE_AGG_TOP_N, CORE_MAX_POSITION_WEIGHT, CORE_STOP1, CORE_STOP2, "AGG"
    elif market_state == "DEF":
        base = CORE_DEF_EXPOSURE
        top_n, max_pos, stop1, stop2, mode = CORE_DEF_TOP_N, CORE_MAX_POSITION_WEIGHT, CORE_STOP1, CORE_STOP2, "DEF"
    else:
        base = CORE_RISK_OFF_EXPOSURE
        top_n, max_pos, stop1, stop2, mode = 0, 0.0, CORE_STOP1, CORE_STOP2, "RISK_OFF"
    ...
    for sym, pos in list(holdings.items()):
        ...
        if mode == "RISK_OFF":
            ...
            continue
        if np.isfinite(gap_ret) and gap_ret <= CORE_STOP2:
            raw_targets[sym] = 0.0
        ...
        if pnl <= CORE_STOP2:
            ...
            continue
        if pnl <= CORE_STOP1 and (not pos["stop1_hit"]):
            pos["weight_mult"] *= 0.5
            pos["stop1_hit"] = True
...
def apply_packet(packet, holdings, cash, day_map, trade_date, trade_rows, sleeve_name):
    ...
    for sym in symbols:
        ...
        elif delta_value > MIN_POSITION_VALUE and can_buy:
    ...
    for sym, close_px, desired_sell in sell_list:
        ...
        if sell_shares <= 0 or trade_value < MIN_POSITION_VALUE:
            continue
    ...
    for sym, close_px, desired_buy in buy_list:
        ...
        if raw_shares <= 0 or trade_value < MIN_POSITION_VALUE or total > cash:
            continue
...
def simulate(df):
    ...
    for date in all_dates:
        ...
        # 強制最低曝險：只在 AGG 市場且不在 hard breaker 時生效
        if market_state == "AGG" and dd > NAV_HARD_DD and total_exp < MIN_TOTAL_EXPOSURE_AGG:
            total_exp = MIN_TOTAL_EXPOSURE_AGG
