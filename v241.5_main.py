...
def simulate(df):
    ...
    for date in sorted(df["trade_date"].unique()):
        ...
        for sym, pos in list(holdings.items()):
            if sym in day["symbol"].values:
                ...
                if pnl <= STOP_LOSS_2:
                    ...
                    trade_rows.append([date,"ADD_LV2",sym,cur])

                elif pnl >= ADD_LV1:
                    holdings[sym]["weight"] *= 1.05
                    trade_rows.append([date,"ADD_LV1",sym,cur])
        ...
        # normalize
        if len(holdings)>0:
            total_w = sum([holdings[s]["weight"] for s in holdings])
            if total_w <= 0:
...
def main():
    ...
    nav.to_csv("v241_5_nav.csv",index=False)
    trades.to_csv("v241_5_trades.csv",index=False)
    summary.to_csv("v241_5_summary.csv",index=False)
