...
def backtest(df):
    ...
    for date in sorted(df["trade_date"].unique()):
        ...
        rows.append([
            date, nav, wA, wB, nA, nB, ret, nav/peak-1, mr, pr
        ])
    ...
    return pd.DataFrame(rows, columns=[
        "date","nav","wA","wB","A_count","B_count","ret","dd","macro","price"
    ])
...
def main():
    ...
    summary = pd.DataFrame([{
        ...
        "avg_wA": out["wA"].mean(),
        "avg_wB": out["wB"].mean(),
        "macro_weak_days": (out["macro"]=="weak").sum()
    }])

    out.to_csv("v232_2_nav.csv", index=False)
    summary.to_csv("v232_2_summary.csv", index=False)
