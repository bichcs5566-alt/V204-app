...
def latest_price_map():
    ...
    for r in rows:
        ...
        close = str(r.get("close") or r.get("adj_close") or r.get("æ¶ç¤å¹") or "").strip()
        ...
        if old is None or d >= old["date"]:
            latest[sid] = {"date": d, "close": close}
    return latest, latest_date

def build_merged():
    current_positions = read_csv(CURRENT_POS)
    watchlist = read_csv(WATCHLIST)
    trade_plan = read_csv(TRADE_PLAN)
    pos_mon = read_csv(POS_MON)
    watch_mon = read_csv(WATCH_MON)
    ...
    for r in current_positions:
        ...
        pos_rows.append({
            "stock_id": sid, "price_tier": pr.get("price_tier") or tr.get("price_tier") or price_tier(ref),
            "ref_price": ref, "shares": str(r.get("shares","")).strip(), "avg_cost": str(r.get("avg_cost","")).strip(),
            "pnl_pct": pr.get("pnl_pct",""), "target_weight": pr.get("target_weight") or tr.get("target_weight",""),
            "current_weight_est": pr.get("current_weight_est",""), "action": pr.get("action") or tr.get("action") or "HOLD",
    ...
    for r in watchlist:
        ...
        watch_rows.append({
            "stock_id": sid, "price_tier": wr.get("price_tier") or tr.get("price_tier") or price_tier(ref),
            "ref_price": ref, "holding_status": wr.get("holding_status","æªææ"),
            "strategy_bucket": wr.get("strategy_bucket") or tr.get("strategy_bucket") or "NONE",
            "action": wr.get("action") or tr.get("action") or "WATCH", "pnl_pct": wr.get("pnl_pct","")
        })
...
def main():
    ...
    if latest_date:
        ...
        meta["trade_date"] = next_trading_day(latest_date).isoformat()
    meta["generated_at"] = now.strftime("%Y-%m-%d %H:%M:%S")
    meta["trade_plan_batch"] = now.strftime("%Y-%m-%d %H:%M:%S")
    meta["data_state"] = "ok"
    meta["source"] = "v266.10.2_pipeline"
    ...
    if diff.returncode != 0:
        subprocess.run(["git","commit","-m","v266.10.2 postprocess meta and merged outputs"], check=True)
