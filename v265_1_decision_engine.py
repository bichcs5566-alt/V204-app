# v265_1_decision_engine.txt
# 覆蓋後改名：v265_1_decision_engine.py
# v265.1（決策完整版）
# 核心：
# - 延續 v264 / v265 主線邏輯
# - 嚴格 T+1
# - 輸出完整決策清單：BUY / SELL / HOLD / REDUCE / STOP_LOSS
# - 提供 current_positions.csv 讓使用者每天維護真實持倉
...
def build_trade_plan(df, latest_capital=INITIAL_CAPITAL):
    ...
    all_symbols = sorted(set(target.keys()) | set(current_map.keys()))
    ...
    if not plan.empty:
        ...
        plan = plan.sort_values(["action_rank", "target_weight"], ascending=[True, False]).drop(columns=["action_rank"])
