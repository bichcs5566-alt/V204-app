# v243_final_backtest.py
# v243 FINAL 盈利驗證版
# 流程：
# 1) 讀取 merge_chunked_price_panel.py 產生的 price_panel_daily.csv
# 2) 讀取 macro_signal.csv（若 workflow 無此檔，會先跑 v235_macro_updater.py）
# 3) 依指定區間回測
# 4) 輸出 nav / trades / signals / summary / meta
...
def parse_args():
    ...
    p.add_argument("--label", required=True, help="output file label")
    return p.parse_args()
...
def load_macro():
    ...
    if "trade_date" not in macro.columns or "macro_score" not in macro.columns:
        raise ValueError("macro_signal.csv 需要 trade_date / macro_score 欄位")
...
def apply_execution_targets(pending_targets, holdings, cash, day_map, trade_date, nav_before_trade, trade_rows):
    ...
    for sym in symbols:
        ...
        current_value = cur_shares * close_px
    ...
    for group, is_buy in [(sell_syms, False), (buy_syms, True)]:
        for sym in group:
            row = day_map.get(sym)
            if row is None:
                continue
            ...
            if not is_buy:
                ...
                sell_shares = min(current_shares, sell_value / max(exec_px, 1e-12))
                ...
                trade_rows.append([trade_date, "SELL", sym, close_px, exec_px, sell_shares, trade_value, fee_tax])
            else:
                if delta_value <= MIN_TRADE_VALUE:
                    continue
                ...
                trade_rows.append([trade_date, "BUY", sym, close_px, exec_px, raw_shares, trade_value, fee_tax])
...
def build_next_day_targets(day_df: pd.DataFrame, holdings: dict):
    ...
    target_n = TOP_N
    if len(active_set) < MIN_HOLDINGS:
        target_n = max(TOP_N, MIN_HOLDINGS)
...
def simulate(df: pd.DataFrame):
    ...
    trade_df = pd.DataFrame(trade_rows, columns=["date", "action", "symbol", "close_px", "exec_px", "shares", "trade_value", "fee_tax"])
