# v251_integrated_engine.txt
# 覆蓋後改名：v251_integrated_engine.py
...
MIN_DAILY_TURNOVER = 2_000_000.0
...
AGG_TOP_N = 6
AGG_MAX_EXPOSURE = 0.85
AGG_MIN_EXPOSURE = 0.35
AGG_MAX_POSITION_WEIGHT = 0.18

DEF_TOP_N = 2
DEF_MAX_EXPOSURE = 0.20
DEF_MIN_EXPOSURE = 0.00
DEF_MAX_POSITION_WEIGHT = 0.12
...
def build_market_state(price):
    ...
    risk_off = (mkt["market_close"] < mkt["ma60"]) | (mkt["market_ret_5d"] < -0.05)
    defensive = (mkt["market_close"] < mkt["ma20"]) | (mkt["market_ret_5d"] < 0)
...
def regime_params(market_state, macro_score):
    if market_state == "AGG":
        base = 0.85 if macro_score > 0.5 else (0.70 if macro_score > 0 else 0.50)
        return {"mode":"AGG","top_n":AGG_TOP_N,"max_exposure":AGG_MAX_EXPOSURE,"min_exposure":AGG_MIN_EXPOSURE,"max_pos_w":AGG_MAX_POSITION_WEIGHT,"stop1":AGG_STOP1,"stop2":AGG_STOP2,"add1":AGG_ADD1,"add2":AGG_ADD2,"addm1":AGG_ADD_MULT1,"addm2":AGG_ADD_MULT2,"base_exposure":min(max(base,AGG_MIN_EXPOSURE),AGG_MAX_EXPOSURE)}
    elif market_state == "DEF":
        base = 0.18 if macro_score > 0 else 0.10
        return {"mode":"DEF","top_n":DEF_TOP_N,"max_exposure":DEF_MAX_EXPOSURE,"min_exposure":DEF_MIN_EXPOSURE,"max_pos_w":DEF_MAX_POSITION_WEIGHT,"stop1":DEF_STOP1,"stop2":DEF_STOP2,"add1":DEF_ADD1,"add2":DEF_ADD2,"addm1":DEF_ADD_MULT1,"addm2":DEF_ADD_MULT2,"base_exposure":min(max(base,DEF_MIN_EXPOSURE),DEF_MAX_EXPOSURE)}
    else:
        return {"mode":"RISK_OFF","top_n":0,"max_exposure":0.0,"min_exposure":0.0,"max_pos_w":0.0,"stop1":DEF_STOP1,"stop2":DEF_STOP2,"add1":DEF_ADD1,"add2":DEF_ADD2,"addm1":1.0,"addm2":1.0,"base_exposure":0.0}


def rank_day(day_df, mode):
    d = day_df.dropna(subset=["mom20", "vol20"]).copy()
    d = d[d["turnover"].fillna(0) >= MIN_DAILY_TURNOVER].copy()
    d = d[d["close"] >= 20].copy()
    if mode == "AGG":
        d = d[(d["mom20"] > 0) & (d["mom10"] > 0)].copy()
        d["score"] = (d["mom20"] / (d["vol20"] + 1e-9)) * 0.65 + d["mom10"] * 0.20 + d["mom5"] * 0.15
    elif mode == "DEF":
        d = d[(d["mom20"] > 0) & (d["mom10"] > 0)].copy()
...
def build_signals_for_next_day(day_df, holdings, nav_basis, dd_now, hard_mode):
    ...
    packet = {"target_weights":normalize_targets(raw_targets, target_exposure, cfg["top_n"], cfg["max_pos_w"]), "nav_basis":float(max(nav_basis,1.0)), "signal_exposure":float(target_exposure), "risk_mode":risk_mode, "hard_mode":hard_mode, "strategy_mode":cfg["mode"], "top_n":cfg["top_n"]}
