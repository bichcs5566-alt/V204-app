# v246_balanced_alpha_engine.txt
# v246 平衡版：保留 v241 alpha，降低 v245 過度保守問題
# 核心：
# 1) 嚴格 T+1，不偷看
# 2) 市場 regime 放寬：MA120 / MA60
# 3) NAV breaker 改為「分級降曝險」而非直接歸零
# 4) 保留基本進場能力：最低曝險下限
# 5) 單檔 / 總曝險仍有限制，避免再次爆倉
...
MIN_ACTIVE_EXPOSURE = 0.20

NAV_SOFT_DD = -0.10
NAV_MEDIUM_DD = -0.15
NAV_HARD_DD = -0.20
NAV_SOFT_EXPOSURE_CAP = 0.60
NAV_MEDIUM_EXPOSURE_CAP = 0.40
NAV_HARD_EXPOSURE_CAP = 0.20
REENTRY_DD = -0.08
...
def build_market_regime(df):
    ...
    cond_weak = mkt["market_close"] < mkt["market_ma60"]
    mkt["market_regime"] = np.where(cond_bear, -1, np.where(cond_weak, 0, 1))
    return mkt[["trade_date", "market_close", "market_ma60", "market_ma120", "market_regime"]]
...
def build_signals_for_next_day(day_df, holdings, nav_basis, dd_now, hard_mode):
    ...
    for sym, pos in list(holdings.items()):
        ...
        if risk_mode in ["NORMAL", "NAV_SOFT"] and market_regime >= 0:
