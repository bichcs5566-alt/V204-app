...
def ensure_price_panel():
    if Path(PRICE_PANEL_FILE).exists(): return
...
def load_watchlist():
    ...
    try:
        df = pd.read_csv(path)
        if "stock_id" not in df.columns: return set()
...
def build_outputs(df):
    ...
    for stock_id in all_symbols:
        ...
        shares = current.get("shares", np.nan)
        ...
        if stock_id in pos_map:
            if pd.notna(pnl_pct) and pnl_pct <= STOP_LOSS_2:
                ...
                if diff > BUY_BAND:
                    action = "ADD"; note = "ä½æ¼ç®æ¨æ¬é"
                elif diff < REDUCE_BAND:
                    action = "REDUCE"; note = "é«æ¼ç®æ¨æ¬é"
                else:
                    action = "HOLD"; note = "æ¬éå¨å®¹è¨±ç¯å"
            else:
            ...
            pos_rows.append({
                ...
                "shares": round(shares, 2) if pd.notna(shares) else "",
