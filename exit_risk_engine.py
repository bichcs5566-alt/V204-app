...
C 完整版：出場與風控引擎。
有持倉檔就產出 SELL / REDUCE / HOLD / WATCH；沒有持倉檔就產出空表，不中斷 pipeline。
"""
...
OUTPUT_COLUMNS = [
    "stock_id", "shares", "avg_cost", "close", "unrealized_pct",
...
def normalize_positions(df):
    ...
    if "avg_cost" not in df.columns:
        if "cost" in df.columns:
            ...
            df["avg_cost"] = df["entry_price"]
        elif "buy_price" in df.columns:
            df["avg_cost"] = df["buy_price"]
        else:
            df["avg_cost"] = np.nan
    ...
    return df[["stock_id", "shares", "avg_cost"]]
...
def main():
    ...
    regime = regime_obj.get("regime", "UNKNOWN")
    ...
    out = pd.DataFrame(rows, columns=OUTPUT_COLUMNS).sort_values(["exit_priority", "stock_id"], ascending=[False, True])
    out.to_csv(ROOT / "exit_risk_plan.csv", index=False, encoding="utf-8")
    out.to_csv(DATA_DIR / "exit_risk_plan.csv", index=False, encoding="utf-8")
    ...
    summary = {
        ...
        "sell_count": int((out["exit_action"] == "SELL").sum()),
        "reduce_count": int((out["exit_action"] == "REDUCE").sum()),
        "watch_count": int((out["exit_action"] == "WATCH").sum()),
        "hold_count": int((out["exit_action"] == "HOLD").sum()),
